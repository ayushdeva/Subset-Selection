	...
	def update(self,qualities,T,round_no):
		for i in range(self.n):
			...
			if qualities[i][1] != 0:
				...
				self.prods[qualities[i][0]].q_pos = new_q +  self.ucb_coeff*math.sqrt((2*math.log(T))/new_freq)
				# self.prods[qualities[i][0]].q_pos = new_q + math.sqrt((2*math.log(round_no))/new_freq)
